...
//포트 초기화 용도
void StepMotor_InitPin(step_port_t *stepPort, GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin)
{
	stepPort->GPIOx=GPIOx;
	stepPort->GPIO_Pin = GPIO_Pin;
}

void StepMotor_WritePin(step_port_t *stepPort, uint8_t state)
{
...
void StepMotor_CW()
{
	...
	{
	case 0:
		StepMotor_WritePin(&stepA1, SET);
		StepMotor_WritePin(&stepB1, SET);
		StepMotor_WritePin(&stepA2, RESET);
		StepMotor_WritePin(&stepB2, RESET);
		break;
	case 1:
		StepMotor_WritePin(&stepA1, RESET);
		StepMotor_WritePin(&stepB1, SET);
		StepMotor_WritePin(&stepA2, SET);
		StepMotor_WritePin(&stepB2, RESET);
		break;
	case 2:
		StepMotor_WritePin(&stepA1, RESET);
		StepMotor_WritePin(&stepB1, RESET);
		StepMotor_WritePin(&stepA2, SET);
		StepMotor_WritePin(&stepB2, SET);
		break;
	case 3:
		StepMotor_WritePin(&stepA1, SET);
		StepMotor_WritePin(&stepB1, RESET);
...
void StepMotor_Speed(uint16_t speed)
{
